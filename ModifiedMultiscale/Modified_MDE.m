function [Out_MMDE]=Modified_MDE(x,m,c,tau,Scale)
%
...
Out_MMDE=NaN*ones(1,Scale);

Out_MMDE(1)=DisEn_NCDF(x,m,c,tau);
...
for j=2:Scale
    xs = Multi(x,j);
    Out_MMDE(j)=DisEn_NCDF_ms(xs,m,c,mu,sigma,tau);
end
end
...
L = length(Data);
M_Data=NaN*ones(1,L-S+1);
for i=1:L-S+1
    M_Data(i) = mean(Data(i:i+S-1));
end
end
